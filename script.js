(() => {
  ...
  const details = {
    banner: {
      name: 'Banner details',
      subtitle: 'Click the banner icon to explore the complete digital experience.',
      features: [
        'Interactive banner controls with keyboard support',
        'Full service and feature details in a focused modal',
        'Direct CTA to start the selected project',
        'Responsive experience on mobile, tablet and desktop'
      ]
    },
    mega: {
      name: 'Mega Scale · ₹80,000',
      subtitle: 'High-performance platform for ambitious teams.',
      features: ['Grade architecture', '120GB high-speed storage', 'Advanced DNS + SSL', '24/7 technical squad support', 'Performance and security optimization']
    },
    custom: {
      name: 'Custom App Ecosystem · ₹100,000',
      subtitle: 'A bespoke application foundation with every critical service connected.',
      features: ['Custom app ecosystem', 'Full block architecture', 'Global service integrations', 'Product design and strategy', 'Launch roadmap and analytics']
    },
    ultimate: {
      name: 'Ultimate Command · ₹150,000',
      subtitle: 'Complete ownership of a full-scale web and app infrastructure.',
      features: ['Custom web and app infrastructure', 'Complete ownership and governance', 'Lifetime support', 'Priority launch and optimization', 'Growth and maintenance partnership']
    }
  };
  ...
  const openDetails = (key) => {
    ...
    modalContent.innerHTML = `
      <div class="modal-header">
        <p class="eyebrow">Full details</p>
      ...
      <a class="modal-cta" href="#contact">Discuss this with us ↗</a>
    `;
  ...
  style.textContent = `
    ...
    .detail-list{list-style:none;padding:0;margin:0 0 22px;display:grid;gap:10px}
    .detail-list li{padding:13px 15px;border:1px solid rgba(92,174,255,.2);background:rgba(255,255,255,.03);border-radius:12px;color:#e6f3ff;font-size:.98rem}
    .modal-cta{display:inline-flex;align-items:center;justify-content:center;padding:14px 22px;border-radius:999px;border:1px solid transparent;background:linear-gradient(135deg,#17d5ff,#5d74ff);color:#fff;font-weight:800;box-shadow:0 18px 44px rgba(91,116,255,.35)}
    @media(max-width:680px){.banner-detail-trigger{right:10px;bottom:10px}.banner-detail-trigger span{display:none}}
  `;
  document.head.appendChild(style);

  const heroImage = $('.hero-image');
  if (heroImage && !heroImage.querySelector('.banner-detail-trigger')) {
    ...
    layersSection.innerHTML = `
      <div class="container">
        <div class="section-heading reveal">
          <p class="eyebrow">The 12-layer system</p>
          <h2>More than a website.<br><em>A complete digital engine.</em></h2>
          <p>Every project is strengthened across twelve connected layers so the experience looks great, works reliably and keeps growing.</p>
        </div>
        <div class="layer-grid">
          ${layers.map(([n, title, text]) => `
            <article class="layer-card reveal">
              <span class="layer-number">LAYER ${n}</span>
              <h3>${title}</h3>
              <p>${text}</p>
            </article>
          `).join('')}
    ...
    featureSection.innerHTML = `
      <div class="container">
        <div class="section-heading reveal">
          ...
          <h2>Every layer is built to produce <em>real business growth.</em></h2>
        </div>
    ...
    roadmap.innerHTML = `
      <div class="container">
        <div class="section-heading reveal">
          ...
          <h2>Built for today.<br><em>Ready for what comes next.</em></h2>
          <p>Our future-ready plan keeps your digital product improving after launch.</p>
        </div>
        <div class="roadmap-grid">
          <article class="roadmap-card reveal"><span>PHASE 01 • NOW</span><h3>Launch foundation</h3><p>Fast, accessible, secure website with analytics and conversion tracking.</p></article>
          <article class="roadmap-card reveal"><span>PHASE 02 • NEXT</span><h3>Automation layer</h3><p>CRM, notifications, smart workflows and dashboards that reduce manual work.</p></article>
          <article class="roadmap-card reveal"><span>PHASE 03 • SCALE</span><h3>Growth intelligence</h3><p>Better experiments, retention strategy and more personalized product experiences.</p></article>
          <article class="roadmap-card reveal"><span>PHASE 04 • FUTURE</span><h3>Digital leadership</h3><p>Expansion into apps, AI systems and global market-ready product infrastructure.</p></article>
        </div>
      </div>
    `;
